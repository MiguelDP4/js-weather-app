export const dom = (() => {
  ...
  const drawWeather = (container, weatherData, temperatureScale) => {
    ...
    const infoDiv = appendHtmlDiv(container, ['w-75', 'd-flex', 'flex-column']);
    const temperatureDiv = appendHtmlDiv(infoDiv, ['w-100', 'd-flex', 'flex-row', 'justify-content-between']);
  ...
  const drawError = (container, message) => {
    container.innerHTML = '';
    appendHtmlHeading(container, message, 2, ['my-3']);
    appendHtmlImage(container, '../img/sad.svg', ['w-75', 'h-auto', 'my-3']);
  };

  return { drawWeather, drawError };
})();
